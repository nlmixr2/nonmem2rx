...
and isn't exactly trivial to calculate from the model output.

One thing you can do is to use model piping to modify the original
model to add this to the imported model:
...
You can also use `append=NA` to prepend or `append=f` to put the ODE
...
In this case since we are also wanting `AUC`, we wish to add a reset
dose to the AUC compartment every time a dose is given (so it will
only track the AUC of the current dose):
