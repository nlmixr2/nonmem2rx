...
When a model has more than one endpoint (the observations have more
than one `DVID`, or without a `DVID` more than one `CMT`), these
datasets also have an `ENDPOINT` column, and the validation plots are
split by endpoint.
