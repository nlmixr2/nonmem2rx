...
- Translate `ADVAN5`/`ADVAN7` general linear models to `rxode2`'s native
  matrix-exponential `matExp()` model instead of ordinary differential
  equations. You can turn it on with `nonmem2rx(..., matexp=TRUE)` or
  `options(nonmem2rx.matexp=TRUE)` (see [General linear models
...
NONMEM solves with matrix exponentials.  By default `nonmem2rx` translates
these to explicit ordinary differential equations:
...
With `matexp=TRUE` the same linear system is translated to `rxode2`'s native
matrix-exponential `matExp()` model instead, using `cmt()` declarations and
`k_<from>_<to>` rate constants (with `k_<from>_output` for elimination):

```{r advan5matexp}
mod <- nonmem2rx(advan5, validate=FALSE, save=FALSE, matexp=TRUE)
mod
```
...
integrator.  This is an opt-in alternative — the default ODE translation is
unchanged, and only `ADVAN5`/`ADVAN7` models are affected.
